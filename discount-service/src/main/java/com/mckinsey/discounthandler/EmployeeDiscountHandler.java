...
public class EmployeeDiscountHandler implements DiscountHandler {
	...
	Logger logger = LoggerFactory.getLogger(EmployeeDiscountHandler.class);
	...
	public double applyDiscount(List<Item> items, String role) {
		...
		if ("employee".equals(role)) {
			logger.info("Calcuating: Employee discount");
			totalDiscount = items.stream().filter(item -> !item.getCategory().equals("grocery")).map(item -> {
				logger.info("Discount on item: {}, discount: {}",item.getName(), item.getPrice() * 0.30);
				return item.getPrice() * 0.30;
			}).collect(Collectors.summingDouble(v -> v));
			logger.info("Employee total discount: {}",totalDiscount);
